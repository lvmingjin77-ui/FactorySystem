...
import 'package:admin/screens/extinguisher/part02/card03.dart';
// import 'package:admin/screens/extinguisher/cell_content2.dart';
...
class FoldingCell203 extends StatefulWidget {
  /// 当卡片关闭或打开时的回调
  final ValueChanged<FoldingState2> onChanged;

  /// 卡片的预设状态
  final FoldingState2 foldingState;

  const FoldingCell203({
    Key? key,
    required this.onChanged,
    this.foldingState = FoldingState2.close,
  }) : super(key: key);
...
class _FoldingCellState extends State<FoldingCell203>
  ...
  void initState() {
    ...
    if (widget.foldingState == FoldingState2.close) {
    ...
    _controller.addStatusListener((status) {
      if (status == AnimationStatus.forward) {
        /// 执行了关闭动画
        widget.onChanged(FoldingState2.close);
      }
      if (status == AnimationStatus.reverse) {
        /// 执行了打开动画
        widget.onChanged(FoldingState2.open);
      }
    });
  }
